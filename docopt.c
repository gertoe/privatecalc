...
"  -v --verbose  print calculation table\n"
"\n"
"";
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_commands; i++) {
        ...
        if (!strcmp(command->name, "gcd")) {
            args->gcd = command->value;
        } else if (!strcmp(command->name, "phi")) {
            args->phi = command->value;
        }
    }
    /* arguments */
    for (i=0; i < elements->n_arguments; i++) {
        argument = &elements->arguments[i];
        if (!strcmp(argument->name, "<a>")) {
            args->a = argument->value;
        } else if (!strcmp(argument->name, "<b>")) {
            args->b = argument->value;
        } else if (!strcmp(argument->name, "<e>")) {
            args->e = argument->value;
        } else if (!strcmp(argument->name, "<n>")) {
            args->n = argument->value;
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    ...
    Elements elements = {2, 5, 3, commands, arguments, options};
